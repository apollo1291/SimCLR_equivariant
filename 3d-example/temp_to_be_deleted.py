...
class ContrastiveDataset(Dataset):
    def __init__(self, ids, X, X3D, noise_std=0.05):
        self.ids = ids
        self.X = X
        self.X3D = X3D
    ...
    def __getitem__(self, idx):
        ...
        with torch.no_grad():
            ...
            return x3d, augmented_x3d, shift, self.X[idx], self.ids[idx]
...
class ContrastiveModel(pl.LightningModule):
    def __init__(self, input_dim, output_dim, lr, alpha):
        ...
        self.proj = nn.Linear(input_dim + 1, output_dim)
    ...
    def forward(self, x, shift):     
        ...
        """
        x_prime = torch.cat([x, torch.zeros(x.size(0), 1, device=x.device)], dim=1)
        x_params = torch.cat([x, shift.view(-1, 1)], dim=1)
    ...
    def training_step(self, batch, batch_idx):
        x3d, augmented_x3d, shift, x2d, ids = batch

        # 1) Compute forward passes
        z1, predicted_z2 = self.forward(x3d, shift)
        z2, predicted_z1 = self.forward(augmented_x3d, -shift)

        # 2) Equivariant part (info-nce on z1 vs predicted_z1 and predicted_z2 vs z2)
        equiv_a = self.info_nce_loss(z1, predicted_z1, torch.eye(z1.shape[0], device=z1.device))
        equiv_b = self.info_nce_loss(predicted_z2, z2, torch.eye(z2.shape[0], device=z2.device))
        equiv_loss = equiv_a + equiv_b
    ...
    def validation_step(self, batch, batch_idx):
        x3d, _, shift, x2d, ids = batch
        z, _ = self.forward(x3d, shift)
...
if __name__ == "__main__":
    ...
    dataset = ContrastiveDataset(ids, X, X3D)
    ...
    for alpha in alpha_values:
        ...
        os.makedirs("saved_runs/epoch_20", exist_ok=True)
        save_path = f"saved_runs/epoch_20/no_noise_model_alpha_{alpha:.2f}.pt"
        torch.save({
            "alpha": alpha,
